...
covid_summary_cases <- function(){
   ...
   covid_cases_summary1 <- data.frame(covid_cases_summary$Countries) 
   #covid_cases_summary2 <- data.frame(covid_cases_summary1)
...
us_covid_cases <- liveCase

us_covid_cases <- aggregate((activeCases = liveCase$Active), 
                            by = list(Date=cut(as.POSIXct(liveCase$Date), "month")), sum)

us_covid_cases


stateCovidCase <- stateCases
stateCovidCase <- aggregate(stateCovidCase$Cases by = list()