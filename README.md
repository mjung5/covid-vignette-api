...
``` r
# Create function to download country level confirmed cases.
...
``` r
# Create function is to download country level death cases.
...
``` r
# Create function to download country level recovered cases.
...
``` r
# Create function to download state level confirmed cases.
...
``` r
# Create function to download state level death cases.
...
``` r
# Create function to download country level live confirmed cases.
...
mainly selected four datasets for the exploratory analysis.
...
``` r
# Subset of the dataset with only death status
...
``` r
# Subset of top 10 states of death cases.
# Descending order
...
``` r
# Subset of top 10 states by death rate.
# Descending order
...
Now, let’s move down to the county level data, beginning with discussing
some summary statistics.

``` r
# Summary stats table for NC counties.
Summary_county_cases <- state_all_cases_month %>% group_by(City, Status) %>%
                summarise(Min = min(Cases), 
                          Median = median(Cases),
                          Average = mean(Cases),
                          Max = max(Cases)
                          )
# Subset of the top 5 counties in NC on confirmed cases.
Summary_county_confirmed <- Summary_county_cases %>% 
                              filter(Status == "confirmed") %>% arrange(desc(Max))
Summary_county_confirmed <- Summary_county_confirmed[1:5,]

# Subset of the top 5 counties in NC on death cases.
Summary_county_deaths <- Summary_county_cases %>% 
                           filter(Status == "deaths") %>% arrange(desc(Max))
Summary_county_deaths <- Summary_county_deaths[1:5,]

# Display a table of the summary stats for top 5 counties on confirmed cases. 
kable(Summary_county_confirmed, caption = "Summary Stats of confirmed cases by top 5 NC county", digits = 2)
```

| City        | Status    |   Min | Median |   Average |    Max |
|:------------|:----------|------:|-------:|----------:|-------:|
| Mecklenburg | confirmed | 40754 | 109948 | 103118.80 | 152437 |
| Wake        | confirmed | 26464 |  85911 |  80022.02 | 124490 |
| Guilford    | confirmed | 15355 |  46460 |  42858.16 |  64984 |
| Forsyth     | confirmed | 12101 |  35263 |  33204.90 |  49690 |
| Cumberland  | confirmed |  8868 |  27907 |  26602.13 |  43017 |

Summary Stats of confirmed cases by top 5 NC county

``` r
# Display a table of the summary stats for top 5 counties on death cases. 
kable(Summary_county_deaths, caption = "Summary Stats of death cases by top 5 NC county", digits = 2)
```

| City        | Status | Min | Median | Average |  Max |
|:------------|:-------|----:|-------:|--------:|-----:|
| Mecklenburg | deaths | 430 |    940 |  873.65 | 1177 |
| Wake        | deaths | 286 |    720 |  620.86 |  852 |
| Guilford    | deaths | 230 |    693 |  598.70 |  840 |
| Gaston      | deaths | 176 |    420 |  389.66 |  576 |
| Forsyth     | deaths | 157 |    374 |  358.54 |  525 |

Summary Stats of death cases by top 5 NC county

The data begins on November 22, 2020 to the present (live data), and the
cases are reported daily. Therefore, the Max column shows the most
recent date’s cases, and the Min column shows the case number on
November 22, 2020.

Looking at the confirmed cases summary table, Mecklenburg County shows
the highest Maximum cases, followed by Wake and Guilford counties. The
confirmed cases of Mecklenburg and Wake Counties are a lot higher than
Guilford County, almost double, in fact. Mecklenburg County is again the
top county of death cases, and Wake County is the second. However, the
discrepancy of the death cases between Mecklenburg County and the
subsequent counties appears smaller than the differences in confirmed
cases. It suggests that even though the confirmed cases are high in
Mecklenburg and Wake Counties, their death rates are comparable to other
counties or even lower. However, We again need to consider the
population of the counties to truly understand the confirmed and death
cases.

In both the confirmed and death cases summary tables, median scores are
similar to average scores, but the average scores are generally lower
than the median scores. The median scores show the confirmed cases and
death cases at the mid-point between November 2020 to present, and the
average scores show the average for all the confirmed and death cases
from the data starting day to the present.

Lastly, I wanted to see how confirmed cases and death cases are changing
in wake county.

``` r
# Subset of rows of Wake county
...
histogram, the highest bar is at around 87,000 confirmed cases. The time
period when the confirmed cases were about 87,000 was most frequent, and
we can interpret this as during those 60 counts (60 days), the confirmed
cases increased slowly, which occurred around the second quarter of the
year. However, confirmed cases are rapidly increasing again because the
